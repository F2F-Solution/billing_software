...
class Purchase_receipt extends MX_Controller {
	...
	if ($this->input->post()) {
	    ...
	    /* $notification = array();
	      ...
	      $created_date = $input['receipt_bill']['created_date'];
	      $days = $input['credit_days'] - 3;
	      $notification['notification_date'] = date('Y-m-d', strtotime('-' . $days . ' days', strtotime($created_date)));
	      $notification['due_date'] = date('Y-m-d', strtotime('+' . $input['credit_days'] . ' days', strtotime($created_date)));
