    ...
    <div class="contentpanel mb-25">
        ...
        <form  action="<?php echo $this->config->item('base_url'); ?>sales_return/make_return/<?php echo $customer['customer_id']; ?>" method="post" class=" panel-body">
                ...
                <table class="table table-striped table-bordered responsive dataTable no-footer dtr-inline" id="add_quotation">
                    ...
                    <thead>
                        ...
                        <tr>
                            ...
                            <td  width="5%" class="first_td1 action-btn-align">Total Price</td>
            ...
            <!--                                    <input type="hidden" id='cat_id' class='form-align cat_id static_style class_req form-control' name='categoty[]' readonly="" value='<?php echo $inv_details['cat_id'] ?>'>
                                ...
                                <td width="">
                                    <input type="hidden" tabindex="-1"    name='per_cost[]' style="width:70px;" class="cost_price percost form-control " value="<?php echo $inv_details['per_cost'] ?>" id="price" readonly=""/>

                                    <input type="text" tabindex="-1"    name='total_sales_amt[]' style="width:70px;" class="total_sales_amt  form-control " value="<?php echo $inv_details['per_cost'] * $inv_details['customer_exists_qty'];?>" id="total_sales_amt" readonly=""/>
                                    ...
                                    <input type="hidden"   tabindex="-1"   name='discount[]' style="width:70px;" class="discount" value="0"/>
                                    <span class="error_msg"></span>
                                </td>



                                <td  width="" class="action-btn-align sgst_td">
...
<script type="text/javascript">
    ...
    $('.avail_qty,.percost,.pertax,.totaltax,.gst,.igst,.discount').live('keyup', function () {
        calculate_function();
    });
    ...
    $('.percost').live('keyup', function () {

        var qty = $(this).closest('tr').find('.qty').val();
        if (Number(qty) == " ") {
            $(this).closest('tr').find('.sales_qty_details .error_msg').text('This field is required');
            return false;
        } else {
            //$(this).closest('tr').attr('data-returnqty', qty);
            $(this).closest('tr').find('.sales_qty_details .error_msg').text(' ');
...

    {
        ...
        $('.return_qty').each(function () {
            ...
            var qty = $(this).closest('tr').find('.avail_qty');
            ...
            var total_return_amt = $(this).closest('tr').find('.total_return_amt');
            ...
            if (Number(rq.val()) != 0 && Number(rq.val()) != '')
            {
                ...
                var tr_class="invid_"+inv_id;

              ;
                $('.total_return_amt'+inv_id+'').val(total_return_val.toFixed(2));
