...
class Purchase_return_model extends CI_Model {
    ...
    public function update_po($purchase_order_id, $data) {
    ...
    public function get_all_pr_details_by_id($id) {
        $this->db->select('erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,'
                ...
                . 'erp_pr_details.per_cost,erp_pr_details.tax,erp_pr_details.gst,erp_pr_details.sub_total,erp_product.model_no,erp_product.product_image,erp_product.type,erp_pr_details.discount,erp_pr_details.igst,erp_pr_details.transport,erp_pr_details.po_id,'
        ...
        //$this->db->join('erp_po_details', 'erp_po_details.po_id=erp_pr_details.po_id', 'LEFT');
        //  $this->db->order_by("erp_pr_details.id", "desc");
        //  $this->db->limit(1);
        // $this->db->order_by("erp_pr_details.id", "desc");
        // $this->db->limit(2);
        ...
        $j = 0;
        foreach ($query as $val) {
            $this->db->select('erp_po_details.quantity');
            $this->db->where('product_id', $val['product_id']);
            $this->db->where('po_id', $val['po_id']);
            $query[$j]['po'] = $this->db->get('erp_po_details')->result_array();
            $j++;
        }
        $i = 0;
        foreach ($query as $val) {
    ...
    public function get_po_details($erp_po_details_id) {
        $this->db->select('erp_po_details.*');
        $this->db->where($this->erp_po_details . '.id', $erp_po_details_id);
    ...
    public function update_po_details($erp_po_details_id, $data) {
        $this->db->where($this->erp_po_details . '.id', $erp_po_details_id);
    ...
    public function get_erp_pr($purchase_order_id) {
        $this->db->select('erp_pr.*');
        $this->db->where($this->erp_pr . '.po_id', $purchase_order_id);
        return $this->db->get($this->erp_pr)->result_array();
    }

    public function update_erp_pr($purchase_order_id, $data) {
//        echo "<pre>";
//        print_r($data);
//        echo $purchase_order_id;
//        exit;
        $this->db->where($this->erp_pr . '.id', $purchase_order_id);
