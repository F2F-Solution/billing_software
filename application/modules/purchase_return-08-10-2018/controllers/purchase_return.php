...
//        exit;
        ...
        foreach ($input['id'] as $key => $val) {
            ...
            $sub_total = $input['po']['subtotal_qty'];
            $net_total = $input['po']['net_total'];
            ...
            if ($return_qty != '') {
                ...
                $data['quantity'] = $already_total_qty - $return_qty;
                $data['sub_total'] = $input['sub_total'][$key];
                $this->purchase_return_model->update_po_details($erp_po_details_id, $data);
                // Erp Product Update..
                $po = $this->purchase_return_model->get_po($purchase_order_id);

                $already_d_qty = $po[0]['delivery_qty'];
                $already_total_quantity = $po[0]['total_qty'];

                $data = array();
                $data['delivery_qty'] = abs($already_d_qty - $return_qty);
                $net_total = $input['po']['net_total'];
                $sub_total_qty = $input['po']['subtotal_qty'];
                $data['net_total'] = $net_total;
                $data['subtotal_qty'] = $sub_total_qty;
                //  $data['total_qty'] = $already_total_quantity - $return_qty;
