...
class Delivery_challan_model extends CI_Model {
    ...
    public function get_all_dc_details_by_id($id) {
        ...
        foreach ($query as $val) {
            ...
            $this->db->select('SUM(sub_total) as return_amt,SUM(return_qty) as return_quantity');
            $this->db->where('sales_return_details.inv_detail_id', $val['id']);
            $return_details = $this->db->get('sales_return_details')->result_array();
            $query[$i]['return_amount'] = $return_details[0]['return_amt'];
            $query[$i]['return_quantity'] = $return_details[0]['return_quantity'];
    ...
    function get_datatables($search_data) {
        ...
        foreach ($query as $key => $data) {
            ...
            $this->db->where('sales_return_details.inv_id', $data->id);
            $return_details = $this->db->get('sales_return_details')->result_array();

            $query[$key]->return_amount = $return_details[0]['return_amt'];

            $query[$key]->return_quantity = $return_details[0]['return_quantity'];

            $query[$key]->payable_amt = $data->net_total - $return_details[0]['return_amt'];
        }
