...
class Sales_return_model extends CI_Model {
    ...
    function get_return_datatables($search_data) {
        ...
        foreach ($query as $key => $data) {
            $this->db->select('SUM(return_qty) as return_quantity,SUM(return_amount) as return_amt');
...
//        die;
    ...
    public function get_all_customer_invoice_history($id, $from_date, $to_date, $product_name) {
        $this->db->select('*');
        $this->db->where('id', $id);
        ...
        $result['advance'] = $get_cutomer_details[0]['advance'];
        ...
        if ($product_name != "") {
            ...
            $this->db->select('erp_invoice_details.product_id');
            $this->db->join('erp_invoice_details', 'erp_invoice_details.in_id=erp_invoice.id');
            $this->db->where_in('erp_invoice_details.product_id', $product_name);
            $this->db->group_by('erp_invoice_details.in_id');
        }
        ...
        foreach ($invoice as $key => $data) {
            ...
            $inv_details = $this->db->get('erp_invoice_details')->result_array();

            if (!empty($inv_details))
                $result['invoice'][$key]['invoice_details'] = $inv_details;
        }
    ...
    public function get_all_invoice_by_id($id) {
        ...
        foreach ($query as $val) {
            ...
            $this->db->select('SUM(total_qty) as total_quantity,SUM(return_qty) as return_quantity,SUM(return_amount) as return_amount,SUM(return_cost_total) as overall_net_total,SUM(sub_total) as basic_total');
