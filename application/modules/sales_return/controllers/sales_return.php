...
class Sales_return extends MX_Controller {
    ...
    function __construct() {
        ...
        $access_arr = array(
            ...
            'sales_return/make_retun_ajax' => 'no_restriction',
            'sales_return/return_bill_copy' => 'no_restriction', // Newly Added..
            'sales_return/sales_return_bill' => 'no_restriction',
            'sales_return/get_category_model_by_invid' => 'no_restriction',
            'sales_return/get_customer_name_bykey' => 'no_restriction',
            'sales_return/get_customer_product_list' => 'no_restriction',
            'sales_return/get_customerid_by_name' => 'no_restriction',
            'sales_return/make_return_view' => 'no_restriction',
        );
    ...
    /* public function index() {
      ...
      $datas['customer_data']=$this->sales_return_model->get_customer_data_by_sales();
    ...
    public function get_customer_name_bykey() {
        $customer_name = $this->input->post('customer_name');

        $data = $this->sales_return_model->get_customer_list_by_keywords($customer_name);
    ...
    public function get_customer_product_list() {
        $cust_id = $this->input->post('cust_id');
        $from_date = $this->input->post('from_date');
        $to_date = $this->input->post('to_date');

        $data = $this->sales_return_model->get_customer_product_list($cust_id, $from_date, $to_date);


        echo json_encode($data);
    }

    public function get_customerid_by_name() {

        $customer_name = $this->input->post('customer_name');

        $data = $this->sales_return_model->get_customerid_by_name($customer_name);
...
//        exit;
    ...
    public function make_return_view($id) {
        ...
        $datas["sales_return_details"] = $sales_return_details = $this->sales_return_model->sales_return_details($id);
//echo "<pre>";print_r($datas['quotation']);exit;
        $datas['company_details'] = $this->project_cost_model->get_company_details_by_firm($id);

        $this->template->write_view('content', 'sales_make_return_views', $datas);
    ...
    public function make_return($id, $from_date, $to_date, $product_name) {
        ...
        if (!empty($input)) {
            ...
            foreach ($invoice_id as $key => $invid) {

                if ($input['return_quantity'][$key] != "0" && $input['return_quantity'][$key] != " ") {

                    $insert['customer'] = $input['customer']['id'];

                    $insert['inv_id'] = $input['inv_id'][$key];

                    $insert['inv_number'] = $invid;

                    $insert['inv_detail_id'] = $input['inv_detail_id'][$key];

                    $insert['cat_id'] = $input['categoty'][$key];

                    if ($input['brand'][$key] == null && $input['brand'][$key] == '') {
                        $brand = 0;
                    } else {
                    ...
                    $insert['total_qty'] = $input['available_quantity'][$key];

                    $insert['return_qty'] = $input['return_quantity'][$key];

                    $insert['per_cost'] = $input['per_cost'][$key];

                    $insert['tax'] = $input['tax'][$key];
                    if ($input['gst'][$key] == null && $input['gst'][$key] == '') {
                        $gst = 0;
                    } else {
                        $gst = $input['gst'][$key];
                    }
                    ...
                    $insert['sub_total'] = $input['return_quantity'][$key] * $input['per_cost'][$key];

                    $insert['return_cost_total'] = $input['sub_total'][$key];
                    $insert['return_amount'] = $input['total_return_amt'][$key];

                    $insert['created_date'] = date('Y-m-d H:i');

                    $insert_arr[] = $insert;

                    $firm_id = $input['firm_id'][$key];

                    $this->sales_return_model->insert_make_return_details($insert);

                    $this->sales_return_model->update_return_qty($insert);

                    $this->sales_return_model->update_stock($insert, $firm_id);
                }
            }

            redirect($this->config->item('base_url') . 'sales_return');
        }


        $from_date = base64_decode($from_date);
        $to_date = base64_decode($to_date);
        $product_name = base64_decode($product_name);
        ...
        if ($product_name != "product_name") {
            $product_name = $explode_product;
        } else {
            $product_name = '';
        }

        $datas['customer'] = $this->sales_return_model->get_all_customer_invoice_history($id, $from_date, $to_date, $product_name);

        $datas["category"] = $category = $this->sales_return_model->get_all_category($id);

        $datas["brand"] = $brand = $this->sales_return_model->get_brand($id);



        $this->template->write_view('content', 'sales_make_return', $datas);
    ...
    public function make_retun_ajax() {
        ...
        $list = $this->sales_return_model->get_return_datatables($search_arr);
        ...
        foreach ($list as $val) {
            ...
            $row[] = $no;
            $row[] = '<a href="' . $this->config->item('base_url') . 'sales_return/make_return_view/' . $val['id'] . '" >' . $val['inv_id'] . '</a>';
//            $row[] = $val['inv_id'];
            ...
            $url = $this->config->item('base_url') . 'sales_return/make_return_view/' . $val['id'];
            $row[] = '<a href="' . $url . '" data-toggle="tooltip" class="tooltips btn btn-default  btn-xs" title="" data-original-title="View"><span class="fa fa-eye"></span></a>';
...
//        die;
    ...
    public function get_category_model_by_invid() {

        $inv_id = $this->input->post('inv_id');
        $data = $this->sales_return_model->get_category_brand_by_invid($inv_id);
        print_r($data);
        exit;
        if ($data != 0)
            echo json_encode($data);
