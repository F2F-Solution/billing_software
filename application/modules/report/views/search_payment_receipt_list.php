...
if (isset($all_receipt) && !empty($all_receipt)) {
    ...
    foreach ($all_receipt as $val) {
        ...
        <tr>
            ...
            <td  class="text_right"><?= number_format($val['invoice_net_total'] - $val['return_amount'], 2, '.', ',') ?></td>
            <td  class="text_right"><?= number_format($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']), 2, '.', ',') ?></td>
            <td  class="text_right"><?= number_format($val['receipt_bill'][0]['receipt_discount'], 2, '.', ',') ?></td>


            <?php $bal = number_format((($val['invoice_net_total'] - $val['return_amount']) - ($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']) + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ','); ?>

            <td  class="text_right"><?= $bal ?></td>
