...
class Report_model extends CI_Model {
    ...
    private $sales_return_details = 'sales_return_details';
    ...
    function get_all_profit_report($serch_data) {
        ...
        foreach ($query as $val) {
            ...
            $query[$i]['payable_amt'] = $val['net_total'] - $return_details[0]['return_amt'];
    ...
    function _get_datatables_query($search_data = array()) {
        ...
        if ($search_data['category'] != '' || $search_data['product'] != '') {
...
//        $this->db->group_by('customer.id');
    ...
    function get_outstanding_datatables($search_data) {
        ...
        $i = 0;
        foreach ($query as $key_d => $duedate) {

            $this->db->select('SUM(return_amount) as return_amt');
            $this->db->where('sales_return_details.inv_id', $duedate['id']);
            $return_details = $this->db->get('sales_return_details')->result_array();

            if (!empty($return_details))
                $query[$i]['return_amount'] = $return_details[0]['return_amt'];
            else
                $query[$i]['return_amount'] = "0.00";
    ...
    function get_customer_return_amt($id, $frim_id, $days_date, $due_date) {
        $this->db->select('SUM(sales_return_details.return_amount) as return_amt,sales_return_details.customer');

        if ($due_date == 0) {
            $this->db->where("DATE_FORMAT(erp_invoice.created_date,'%Y-%m-%d') <'" . $days_date . "'");
        } elseif ($days_date != 0 && $due_date != 0) {
            $this->db->where("DATE_FORMAT(erp_invoice.created_date,'%Y-%m-%d') >='" . $days_date . "' AND DATE_FORMAT(erp_invoice.created_date,'%Y-%m-%d') <= '" . $due_date . "'");
        }

        $this->db->where('sales_return_details.customer', $id);
        $this->db->where('sales_return_details.return_qty !=', 0);
        $this->db->join('erp_invoice', 'erp_invoice.id=sales_return_details.inv_id');
        //$this->db->group_by('sales_return_details.inv_id');
        $query = $this->db->get('sales_return_details')->result_array();
        return $query[0]['return_amt'];
    }
    ...
    function get_outstanding_duedate_datatables($search_data) {
        ...
        foreach ($query as $key => $val) {
            ...
            //$this->db->select('*');
            $this->db->select('SUM(balance) as new_balance,inv_id,invoice_amt,paid_amt');
            ...
            $day_results = $this->db->get()->result_array();
            $day_results[0]['return_amt'] = round($this->get_customer_return_amt($val['id'], $frim_id, $seven_date, $due_date));
            $query[$i]['days'] = $day_results;
            ...
            $this->db->group_by('payment_report.customer');
            $this->db->from('payment_report');
            $sevendays = $this->db->get()->result_array();
            $sevendays[0]['return_amt'] = round($this->get_customer_return_amt($val['id'], $frim_id, $thirty_date, $seven_date1));
            $query[$i]['sevendays'] = $sevendays;
            ...
            $this->db->group_by('payment_report.customer');
            $this->db->from('payment_report');
            $thirtydays = $this->db->get()->result_array();
            $thirtydays[0]['return_amt'] = round($this->get_customer_return_amt($val['id'], $frim_id, $ninty_date, $thirty_date1));
            $query[$i]['thirtydays'] = $thirtydays;
            ...
            $this->db->group_by('payment_report.customer');
            $this->db->from('payment_report');
            $nintydays = $this->db->get()->result_array();
            $thirty_date1 = "0";
            $nintydays[0]['return_amt'] = round($this->get_customer_return_amt($val['id'], $frim_id, $ninty_date, $thirty_date1));
            $query[$i]['nintydays'] = $nintydays;
            ...
            $rec = $this->db->get('payment_report')->result_array();
            $thirty_date1 = "0";
            $ninty_date = 0;
            $rec[0]['return_amt'] = round($this->get_customer_return_amt($val['id'], $frim_id, $ninty_date, $thirty_date1));
            $query[$i]['rec'] = $rec;

            $query[$i]['advance_inv_count'] = $this->get_customer_advance_count($val['id'], $frim_id);
//            echo $this->db->last_query();
//            exit;
            $i++;
        }
    ...
    function get_customer_advance_count($id, $frim_id, $days_date, $due_date) {

        $this->db->select('erp_invoice.id');
        $this->db->where('erp_invoice.customer', $id);
        $this->db->where_in('erp_invoice.firm_id', $frim_id);
        $advane_amt = $this->db->get('erp_invoice')->result_array();
        return $query[0]['advance_inv_count'] = count($advane_amt);
    }
...
//        $this->db->where('erp_invoice.customer',1087);
        ...
        if (!empty($result_arr)) {
            foreach ($result_arr as $value) {
                $this->db->distinct();
                $this->db->select('erp_invoice.net_total AS net_total,erp_invoice.id as invId');
        ...
        if (!empty($result_arr)) {
            foreach ($result_arr as $value) {
                ...
                $this->db->select('erp_invoice.net_total,erp_invoice.id as invId,erp_invoice.inv_id');
                ...
                $this->db->where('erp_invoice.firm_id', $value['firm_id']);
                $advane_amt = $this->db->get('erp_invoice')->result_array();
                $result_arr[$i]['advance_inv_count'] = count($advane_amt);

                foreach ($sub_query as $total) {
                    $this->db->select('SUM(return_amount) as return_amt');
                    $this->db->where('sales_return_details.inv_id', $total['invId']);
                    $return_details = $this->db->get('sales_return_details')->result_array();

                    $result_arr[$i]['return_amount'] = $return_details[0]['return_amt'];

                    $result_arr[$i]['payable_amt'] = $total['net_total'] - $return_details[0]['return_amt'];
                }
    ...
    public function get_customer_based_datatables($serch_data = array()) {
        ...
        foreach ($query as $val) {
            ...
            $query[$i]['return_amt'] = $return_details[0]['return_amount'];
...
//                $this->db->where($this->customer . '.tin IS NULL');
        ...
        $this->db->select('customer.id as customer,customer.advance,customer.store_name,customer.tin,customer.state_id, customer.name,customer.mobil_number,customer.email_id,customer.address1,erp_invoice.id,erp_invoice.inv_id,erp_quotation.q_no,erp_invoice.total_qty,erp_invoice.tax,erp_quotation.ref_name,erp_invoice.tax_label,'
    ...
    public function get_customer_details($search_data = array()) {
        ...
        $this->db->order_by("customer.store_name", "asc");
...
//        echo $this->db->last_query();
    ...
    public function outstanding_report($serch_data = NULL) {
        ...
        $this->db->distinct('customer.id');
        $this->db->select('erp_invoice.firm_id,erp_invoice.id as Invid');
        $this->db->select('customer.name,customer.store_name,customer.mobil_number,customer.advance,customer.id');
        $this->db->join('customer', 'customer.id=erp_invoice.customer', 'left');
//        $this->db->group_by('customer.id');
        ...
        foreach ($query as $val) {
            ...
            $this->db->join('receipt_bill', 'receipt_bill.receipt_id=erp_invoice.id', 'left');
            $sub_query = $this->db->get('erp_invoice')->result_array();
            $query[$i]['receipt_bill'] = $sub_query;

            $this->db->select('erp_invoice.id');
            $this->db->where('erp_invoice.customer', $val['id']);
            $this->db->where('erp_invoice.firm_id', $val['firm_id']);
            $advane_amt = $this->db->get('erp_invoice')->result_array();
            $query[$i]['advance_inv_count'] = count($advane_amt);
            ...
            foreach ($sub_query as $total) {
                ...
                $query[$i]['payable_amt'] = $total['net_total'] - $return_details[0]['return_amt'];
            }
...
//            $j++;
    ...
    function get_commission_report() {
        $this->_get_commission_datatables_query($search_data);
    ...
    function _get_commission_datatables_query($search_data = array()) {
    ...
    function count_all_commission_report() {
        $this->db->from('erp_invoice');
        return $this->db->count_all_results();
    }

    function count_filtered_commssion_report() {
        $this->_get_commission_datatables_query();
        $query = $this->db->get('erp_invoice');
        return $query->num_rows();
    }

    function get_sales_return_list($search_data) {
        ...
        $this->db->select('sales_return_details.*,erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,customer.store_name,customer.state_id,erp_invoice.net_total,customer.firm_id,customer.tin,erp_manage_firms.gstin,customer.advance,,erp_manage_firms.firm_name');
        $this->db->join('erp_category', 'erp_category.cat_id=sales_return_details.cat_id', 'left');
        $this->db->join('erp_product', 'erp_product.id=sales_return_details.product_id');
        $this->db->join('erp_brand', 'erp_brand.id=sales_return_details.brand_id', 'left');
        $this->db->join('customer', 'customer.id=sales_return_details.customer', 'left');
        $this->db->join('erp_invoice', 'erp_invoice.id=sales_return_details.inv_id', 'left');
        $this->db->join('erp_manage_firms', 'erp_manage_firms.firm_id=erp_invoice.firm_id');
        $this->db->where('sales_return_details.return_qty !=', 0);
        $this->db->where_in('erp_invoice.firm_id', $frim_id);
        if (isset($search_data) && !empty($search_data)) {
            ...
            if (isset($search_data["from_date"]) && $search_data["from_date"] != "" && isset($search_data["to_date"]) && $search_data["to_date"] != "") {
                ...
                $this->db->where("DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') >='" . $search_data["from_date"] . "' AND DATE_FORMAT(erp_invoice.created_date,'%Y-%m-%d') <= '" . $search_data["to_date"] . "'");
            } elseif (isset($search_data["from_date"]) && $search_data["from_date"] != "" && isset($search_data["to_date"]) && $search_data["to_date"] == "") {

                $this->db->where("DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') >='" . $search_data["from_date"] . "'");
            } elseif (isset($search_data["from_date"]) && $search_data["from_date"] == "" && isset($search_data["to_date"]) && $search_data["to_date"] != "") {
                $this->db->where("DATE_FORMAT(erp_invoice.created_date,'%Y-%m-%d') <= '" . $search_data["to_date"] . "'");
            }
            if (!empty($search_data['inv_id']) && $search_data['inv_id'] != 'Select') {
                $this->db->where('sales_return_details.inv_number', $search_data['inv_id']);
            }
            if (!empty($search_data['customer']) && $search_data['customer'] != 'Select') {
                $this->db->where('sales_return_details.customer', $search_data['customer']);
            }
            if (!empty($search_data['product']) && $search_data['product'] != 'Select') {
                $this->db->where('sales_return_details.product_id', $search_data['product']);
            }
            if (!empty($search_data['firm']) && $search_data['firm'] != 'Select') {

                $this->db->where('erp_invoice.firm_id', $search_data['firm']);
            }
            $where_gst = '(sales_return_details.tax="' . $search_data['gst'] . '" OR sales_return_details.gst = "' . $search_data['gst'] . '")';
            if (!empty($search_data['gst']) && $search_data['gst'] != 'Select') {
                $this->db->where($where_gst);
            }
        }
        ...
        foreach ($query as $val) {
            $this->db->select('SUM(discount) AS receipt_discount,SUM(bill_amount) AS receipt_paid,MAX(due_date) AS next_date,MAX(created_date) AS paid_date');
            $this->db->where('receipt_bill.receipt_id', $val['inv_id']);
            $query[$j]['receipt_bill'] = $this->db->get('receipt_bill')->result_array();
            $j++;
        }
        return $query;
    ...
    function get_all_customer_invoice_details() {
    ...
    public function get_return_report_datatables() {
        $this->db->select('sales_return_details.*,erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,customer.state_id,customer.store_name');
        $this->db->join('erp_category', 'erp_category.cat_id=sales_return_details.cat_id', 'left');
        $this->db->join('erp_product', 'erp_product.id=sales_return_details.product_id');
        $this->db->join('erp_brand', 'erp_brand.id=sales_return_details.brand_id', 'left');
        $this->db->join('customer', 'customer.id=sales_return_details.customer', 'left');
        ...
        if (isset($search_data) && !empty($search_data)) {
            ...
            if (isset($search_data["from_date"]) && $search_data["from_date"] != "" && isset($search_data["to_date"]) && $search_data["to_date"] != "") {
                ...
                $this->db->where("DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') >='" . $search_data["from_date"] . "' AND DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') <= '" . $search_data["to_date"] . "'");
            } elseif (isset($search_data["from_date"]) && $search_data["from_date"] != "" && isset($search_data["to_date"]) && $search_data["to_date"] == "") {

                $this->db->where("DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') >='" . $search_data["from_date"] . "'");
            } elseif (isset($search_data["from_date"]) && $search_data["from_date"] == "" && isset($search_data["to_date"]) && $search_data["to_date"] != "") {
                $this->db->where("DATE_FORMAT(sales_return_details.created_date,'%Y-%m-%d') <= '" . $search_data["to_date"] . "'");
            }
            if (!empty($search_data['customer']) && $search_data['customer'] != 'Select') {
                $this->db->where('sales_return_details.customer', $search_data['customer']);
            }
        ...
        $i = 0;
        $column_order = array(null, 'customer.store_name', 'sales_return_details.inv_number', 'erp_category.categoryName', 'erp_product.product_name', 'erp_brand.brands', 'sales_return_details.total_qty', 'sales_return_details.return_cost_total', 'sales_return_details.return_qty', 'sales_return_details.sub_total', 'sales_return_details.gst', 'sales_return_details.igst', 'sales_return_details.created_date');
        $column_search = array(null, 'customer.store_name', 'sales_return_details.inv_number', 'erp_category.categoryName', 'erp_product.product_name', 'erp_brand.brands', 'sales_return_details.total_qty', 'sales_return_details.return_cost_total', 'sales_return_details.return_qty', 'sales_return_details.sub_total', 'sales_return_details.gst', 'sales_return_details.igst', 'sales_return_details.created_date');
        //$this->db->where('sales_return_details.return_qty !=', 0);
        $order = array('sales_return_details.id' => 'DESC');
        foreach ($column_search as $item) { // loop column
        ...
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get('sales_return_details');
        return $query->result();
    }

    public function count_all_return_report() {
        $this->db->from($this->sales_return_details);
        return $this->db->count_all_results();
    }

    public function count_filtered_return_report($search_data) {
        $this->db->select('sales_return_details.*,erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,customer.state_id,customer.store_name');
        $this->db->join('erp_category', 'erp_category.cat_id=sales_return_details.cat_id', 'left');
        $this->db->join('erp_product', 'erp_product.id=sales_return_details.product_id');
        $this->db->join('erp_brand', 'erp_brand.id=sales_return_details.brand_id', 'left');
