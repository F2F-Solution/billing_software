...
class Report extends MX_Controller {
    ...
    function __construct() {
        ...
        $access_arr = array(
            ...
            'report/commission_report_ajaxList' => 'no_restriction',
            'report/commission_report_list' => 'no_restriction',
            'report/sales_return_report' => 'no_restriction',
            'report/sales_return_search_result' => 'no_restriction',
            'report/sales_return_ajaxList' => 'no_restriction',
            'report/getall_sales_return_entries' => 'no_restriction',
        );
...
//        exit;
    ...
    function export_all_profit_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Customer Name', 'Invoice No', 'Invoice Date', 'Invoice Amount', 'Return Amount', 'Total Amount', 'Commission Amount', 'Original Amount', 'Profit', 'Profit Amount'));
    ...
    function profit_ajaxList() {
        ...
        foreach ($list as $val) {
            ...
            $payable_amount = $invoice_total - $val['return_amount'];
            ...
            $row[] = ($net_total > 0 ) ? number_format($net_total, 2) : '0';

            $row[] = number_format($val['return_amount'], 2);

            $row[] = number_format($val['payable_amt'], 2);
    ...
    function export_all_firmwise_csv($query, $timezones = array()) {
        ...
        foreach ($query as $key => $customer) {
            ...
            if ($customer['return_amount'] != "")
                $return_amt = $customer['return_amount'];
            else
                $return_amt = 0;



            $row = array($key + 1, ($customer['store_name']) ? $customer['store_name'] : $customer['name'], $customer['mobil_number'], ($customer['firm_id'] == 1) ? number_format(($customer['net_total'] - $return_amt) - ($customer['electricals'] + $customer['advance']), 2) : '0', ($customer['firm_id'] == 2) ? number_format(($customer['net_total'] - $return_amt) - ($customer['paints'] + $customer['advance']), 2) : '0', ($customer['firm_id'] == 3) ? number_format(($customer['net_total'] - $return_amt) - ($customer['tiles'] + $customer['advance']), 2) : '0', ($customer['firm_id'] == 4) ? number_format(($customer['net_total'] - $return_amt) - ($customer['hardware'] + $customer['advance']), 2) : '0', ($customer['net_total'] != '') ? number_format(($customer['net_total'] - $return_amt) - ($customer['net_amount'] + $customer['advance']), 2) : '0');
    ...
    function outstanding_ajaxList() {
        ...
        foreach ($list as $customer) {
            ...
            $row[] = ($customer['firm_id'] == 1) ? number_format(($customer['net_total'] - $return_amt) - ($customer['electricals'] + $customer['advance']), 2) : '0';

            $row[] = ($customer['firm_id'] == 2) ? number_format(($customer['net_total'] - $return_amt) - ($customer['paints'] + $customer['advance']), 2) : '0';

            $row[] = ($customer['firm_id'] == 3) ? number_format(($customer['net_total'] - $return_amt) - ($customer['tiles'] + $customer['advance']), 2) : '0';

            $row[] = ($customer['firm_id'] == 4) ? number_format(($customer['net_total'] - $return_amt) - ($customer['hardware'] + $customer['advance']), 2) : '0';

            $row[] = ($customer['net_total'] != '') ? number_format(($customer['net_total'] - $return_amt) - ($customer['net_amount'] + $customer['advance']), 2) : '0';
        ...
        foreach ($query as $key => $customer) {
            ...
            if (!empty($customer['days'])) {
                ...
                foreach ($customer['days'] as $days_value) {
                    ...
                    if ($days_value['new_balance'] != 0) {
                        $days_net_total = $days_net_total + (($days_value['new_balance'] - $days_value['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
            }
            ...
            if (!empty($customer['sevendays'])) {
                ...
                foreach ($customer['sevendays'] as $seven_days_value) {
                    ...
                    if ($seven_days_value['new_balance'] != 0) {
                        // $seven_days_net_total = $seven_days_net_total + (($seven_days_value['invoice_amt'] - $seven_days_value['return_amt']) - $seven_days_value['paid_amt']);
                        $seven_days_net_total = $seven_days_net_total + (($seven_days_value['new_balance'] - $seven_days_value['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
            }
            ...
            if (!empty($customer['thirtydays'])) {
                ...
                foreach ($customer['thirtydays'] as $thirty_days_value) {
                    ...
                    if ($thirty_days_value['new_balance'] != 0) {
                        //$thirty_days_net_total = $thirty_days_net_total + (($thirty_days_value['invoice_amt'] - $thirty_days_value['return_amt']) - $thirty_days_value['paid_amt']);
                        $thirty_days_net_total = $thirty_days_net_total + (($thirty_days_value['new_balance'] - $thirty_days_value['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
            }
            ...
            if (!empty($customer['nintydays'])) {
                ...
                foreach ($customer['nintydays'] as $ninty_days_value) {
                    ...
                    if ($ninty_days_value['new_balance'] != 0) {
                        //$ninty_days_net_total = $ninty_days_net_total + (($ninty_days_value['invoice_amt'] - $ninty_days_value['return_amt']) - $ninty_days_value['paid_amt']);
                        $ninty_days_net_total = $ninty_days_net_total + (($ninty_days_value['new_balance'] - $ninty_days_value['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
            }
            ...
            if (!empty($customer['rec'])) {
                ...
                foreach ($customer['rec'] as $wingsinvoice) {
                    ...
                    if ($wingsinvoice['new_balance'] != 0) {
                        //$wingsinvoice_net_total += (($wingsinvoice['invoice_amt'] - $wingsinvoice['return_amt']) - $wingsinvoice['paid_amt']);
                        $wingsinvoice_net_total += (($wingsinvoice['new_balance'] - $wingsinvoice['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
            }
        ...
        foreach ($list as $customer) {
            ...
            if (!empty($customer['days'])) {
                ...
                foreach ($customer['days'] as $days_value) {
                    ...
                    // $days_net_total = $days_net_total + (($days_value['invoice_amt'] - $days_value['return_amt']) - $days_value['paid_amt']);
                    if ($days_value['new_balance'] != 0) {
                        $days_net_total = $days_net_total + (($days_value['new_balance'] - $days_value['return_amt'] - ($customer['advance_inv_count'] * $customer['advance'])));
                    }
                }
...
//            $row[] = ($ninty_days_net_total <= 0 ) ? '' : number_format($ninty_days_net_total, 2);
    ...
    function outstanding_excel_report() {
        ...
        //  echo "<pre>";print_r($outstanding);exit;
    ...
    function export_all_outstanding_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Customer Name', 'Inv Amt', 'Return Amt', 'Payable Amt', 'Paid Amt', 'Discount Amt', 'Balance', 'Created Date', 'Due Date'));
        ...
        foreach ($query as $key => $val) {
            ...
            $row = array($key + 1, ($val['store_name']) ? $val['store_name'] : $val['name'], number_format($val['invoice_net_total'], 2, '.', ','), number_format($val['return_amount'], 2, '.', ','), number_format(($val['invoice_net_total'] - $val['return_amount']), 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']), 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_discount'], 2, '.', ','), number_format((($val['invoice_net_total'] - $val['return_amount']) - ($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']) + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ','), ($val['receipt_bill'][0]['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['created_date'])) : '-', ($val['receipt_bill'][0]['next_date'] != '') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['next_date'])) : '');
...
//            $inv= array_sum($val['invoice_net']);
            ...
            $balance_status = number_format((($val['invoice_net_total'] - $val['return_amount']) - ($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']) + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ',');
...
//            $row[] = number_format($val['receipt_bill'][0]['net_total'], 2, '.', ',');
            ...
            $row[] = number_format($val['invoice_net_total'] - $val['return_amount'], 2, '.', ',');

            $row[] = number_format($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']), 2, '.', ',');

            $row[] = number_format($val['receipt_bill'][0]['receipt_discount'], 2, '.', ',');

            $row[] = number_format((($val['invoice_net_total'] - $val['return_amount']) - ($val['receipt_bill'][0]['receipt_paid'] + ($val['advance_inv_count'] * $val['advance']) + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ',');
    ...
    function export_all_customer_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Customer Name', 'Inv', 'Inv Amt', 'Advance Amt', 'Paid Amt', 'Return Amt', 'Discount Amt', 'Balance', 'Inv Date', 'Paid Date'));
...
//            }
            ...
            if (($val['return'][0]['id'] != $val['return'][1]['id'])) {
                ...
                $balance = number_format(($val['net_total'] - $val['return_amt']) - ($val['receipt_bill'][0]['receipt_paid'] + $val['receipt_bill'][0]['receipt_discount'] + $val['advance']), 2, '.', ',');
            } else {
            ...
            $row = array($key + 1, $val['store_name'], $val['inv_id'], number_format($val['net_total'], 2, '.', ','), number_format($val['advance'], 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_paid'], 2, '.', ','), number_format($val['return_amt'], 2, '.', ','), $discount_amt, number_format((($val['net_total'] - $val['return_amt']) - round(($val['receipt_bill'][0]['receipt_paid']) + ($discount_amt + $val['advance']), 2)), 2), ($val['created_date'] != '' && $val['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['created_date'])) : '-', ($val['receipt_bill'][0]['next_date'] != '') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['next_date'])) : '');
...
//        exit;
        ...
        foreach ($list as $val) {
            ...
            $discount = number_format($val['receipt_bill'][0]['receipt_discount'], 2, '.', ',');

            $row[] = number_format((($val['net_total'] - $val['return_amt']) - round($val['receipt_bill'][0]['receipt_paid'] + ($discount + $val['advance']), 2)), 2);
    ...
    function export_all_inv_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Invoice Id', 'Customer Name', 'Total Qty', 'CGST', 'SGST', 'Sub Total', 'Invoice Amt', 'Paid Amt', 'Return Amt', 'Balance Amt', 'Invoice Date', 'Piad Date', 'Credit Days', 'Due Date', 'Credit Limit', 'Exceeded Credit Limit', 'Sales Man'));
...
//        }
    ...
    function export_all_quotation_csv($query, $timezones = array()) {
        ...
        foreach ($query[quotation] as $key => $val) {
            ...
            $return_amount = number_format($val['return_amt'], 2, '.', ',');

            $balance = number_format((($val['net_total'] - $val['return_amt']) - round($val['receipt_bill'][0]['receipt_paid'] + $val['advance'] + $val['receipt_bill'][0]['receipt_discount'], 2)), 2);

            $row = array($key + 1, $val['inv_id'], $val['firm_name'], ($val['gstin']) ? $val['gstin'] : '', ($val['store_name']) ? $val['store_name'] : $val['name'], ($val['tin']) ? $val['tin'] : '', number_format($val['total_qty']), number_format(($val['erp_invoice_details'][0]['cgst']), 2), number_format(($val['erp_invoice_details'][0]['sgst']), 2), number_format($val['subtotal_qty'], 2), number_format($val['net_total'], 2), ($val['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['created_date'])) : '');
...
//            $row[] = number_format($val['return_amt'], 2, '.', ',');
//
//            $row[] = number_format((($val['net_total'] - $val['return_amt']) - round($val['receipt_bill'][0]['receipt_paid'] + $val['advance'] + $val['receipt_bill'][0]['receipt_discount'], 2)), 2);
//            $row[] = number_format(($val['receipt_bill'][0]['receipt_paid'] + $val['advance']), 2, '.', ',');
...
//            $row[] = $val['sales_man_name'];
    ...
    function export_all_hr_csv($query, $timezones = array()) {
        ...
        foreach ($query as $key => $val) {
            ...
            $row = array($key + 1, $val['inv_id'], ($val['store_name']) ? $val['store_name'] : $val['name'], number_format($val['total_qty']), number_format($val['net_total'], 2), ($val['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['created_date'])) : '', $val['credit_days'] > 0 ? $val['credit_days'] : '-', $due_date, ($val['credit_limit'] != '') ? $val['credit_limit'] : '-', ($val['exceeded_limit'] != '') ? $val['exceeded_limit'] : '-');
...
//    }
    ...
    public function commission_report_list() {

        $datas = array();



        $this->template->write_view('content', 'report/commission_report', $datas);

        $this->template->render();
    }

    function commission_report_ajaxList() {
        ...
        $list = $this->report_model->get_commission_report($search_arr);
...
//            }
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->report_model->count_all_commission_report(),
            "recordsFiltered" => $this->report_model->count_filtered_commssion_report(),
            "data" => $data,
        );
    ...
    function sales_return_report() {
        ...
        $data['customer'] = $this->report_model->get_all_customer_invoice_details();

        $data['firms'] = $firms = $this->user_auth->get_user_firms();

        $this->template->write_view('content', 'report/sales_return_report', $data);

        $this->template->render();
    }

    public function sales_return_search_result() {

        $search_data = $this->input->get();
        $data['all_style'] = $this->report_model->get_all_invoice();
        $data['search_data'] = $search_data;

        $data['customer'] = $this->report_model->get_sales_return_list($search_data);

        $this->load->view('report/sales_return_search_list', $data);
    }

    public function sales_return_ajaxList() {

        $search_data = array();

        $search_data = $this->input->post();

        $list = $this->report_model->get_return_report_datatables($search_data);
        ...
        foreach ($list as $ass) {
            ...
            $row[] = $ass->return_cost_total ? $ass->return_cost_total : '0.00';

            $row[] = $ass->return_qty;

            $row[] = $ass->sub_total ? $ass->sub_total : '0.00';

            $row[] = $ass->gst;

            $row[] = $ass->igst;

            $row[] = ($ass->created_date != '' && $ass->created_date != '1970-01-01') ? date('d-M-Y', strtotime($ass->created_date)) : '-';
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->report_model->count_all_return_report(),
            "recordsFiltered" => $this->report_model->count_filtered_return_report($search_data),
            "data" => $data,
        );
    ...
    function getall_sales_return_entries() {

        $search_data = array();

        $search_data = $this->input->get();

        $sales_return_list = $this->report_model->get_sales_return_list($search_data);



        $this->export_all_expenses_csv($sales_return_list);
    }
    ...
    function export_all_expenses_csv($query, $timezones = array()) {
        ...
        header('Content-Disposition: attachment; filename=Sales Return Report.csv');
        ...
        fputcsv($output, array('S.No', 'Invoice ID', 'Firm Name', 'Firm GSTIN', 'Customer name', 'Customer GSTIN', 'Inv Amt', 'Return QTY', 'CGST %', 'SGST%', 'Subtotal', 'Return Amt', 'Balance Amt', 'Return Date'));
        ...
        foreach ($query as $key => $value) {
            ...
            $row = array($key + 1, $value['inv_number'], $value['firm_name'], $value['gstin'], $value['store_name'], $value['tin'], $value['net_total'], $value['return_qty'], number_format(($value['sub_total'] * $value['tax'] / 100), 2), number_format(($value['sub_total'] * $value['gst'] / 100), 2), number_format($value['sub_total'], 2), $value['return_amount'], number_format((($value['net_total'] - $value['return_amount']) - round($value['receipt_bill'][0]['receipt_paid'] + $value['advance'] + $value['receipt_bill'][0]['receipt_discount'], 2)), 2), ($value['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($value['created_date'])) : '');
