        ...
        foreach ($query as $key => $customer) {
            ...
            if (!empty($customer['days'])) {
                ...
                foreach ($customer['days'] as $days_value) {
                    ...
                    $days_net_total = $days_net_total + (($days_value['new_balance'] - $days_value['return_amt']));
                }
            }
            ...
            if (!empty($customer['sevendays'])) {
                ...
                foreach ($customer['sevendays'] as $seven_days_value) {
                    ...
                    $seven_days_net_total = $seven_days_net_total + (($seven_days_value['new_balance'] - $seven_days_value['return_amt']));
                }
            }
            ...
            if (!empty($customer['thirtydays'])) {
                ...
                foreach ($customer['thirtydays'] as $thirty_days_value) {
                    ...
                    $thirty_days_net_total = $thirty_days_net_total + (($thirty_days_value['new_balance'] - $thirty_days_value['return_amt']));
                }
            }
            ...
            if (!empty($customer['nintydays'])) {
                ...
                foreach ($customer['nintydays'] as $ninty_days_value) {
                    ...
                    $ninty_days_net_total = $ninty_days_net_total + (($ninty_days_value['new_balance'] - $ninty_days_value['return_amt']));
                }
            }
            ...
            if (!empty($customer['rec'])) {
                ...
                foreach ($customer['rec'] as $wingsinvoice) {
                    ...
                    $wingsinvoice_net_total += (($wingsinvoice['new_balance'] - $wingsinvoice['return_amt']));
                }
            }
...
//            $row[] = ($ninty_days_net_total <= 0 ) ? '' : number_format($ninty_days_net_total, 2);
    ...
    function export_all_outstanding_csv($query, $timezones = array()) {
        ...
        foreach ($query as $key => $val) {
            ...
            $row = array($key + 1, ($val['store_name']) ? $val['store_name'] : $val['name'], number_format($val['invoice_net_total'], 2, '.', ','), number_format($val['return_amount'], 2, '.', ','), number_format($val['payable_amt'], 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_paid'], 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_discount'], 2, '.', ','), number_format(($val['payable_amt'] - ($val['receipt_bill'][0]['receipt_paid'] + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ','), ($val['receipt_bill'][0]['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['created_date'])) : '-', ($val['receipt_bill'][0]['next_date'] != '') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['next_date'])) : '');
...
//            $row[] = number_format($val['receipt_bill'][0]['net_total'], 2, '.', ',');
            ...
            $row[] = number_format(($val['payable_amt'] - ($val['receipt_bill'][0]['receipt_paid'] + $val['receipt_bill'][0]['receipt_discount'])), 2, '.', ',');
...
//            }
            ...
            $row = array($key + 1, $val['store_name'], $val['inv_id'], number_format($val['net_total'], 2, '.', ','), number_format($val['advance'], 2, '.', ','), number_format($val['receipt_bill'][0]['receipt_paid'], 2, '.', ','), number_format($val['return_amt'], 2, '.', ','), $discount_amt, number_format((($val['net_total'] - $val['return_amt']) - $val['receipt_bill'][0]['receipt_paid']), 2), ($val['created_date'] != '' && $val['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['created_date'])) : '-', ($val['receipt_bill'][0]['next_date'] != '') ? date('d-M-Y', strtotime($val['receipt_bill'][0]['next_date'])) : '');
...
//        exit;
        ...
        foreach ($list as $val) {
            ...
            $row[] = number_format((($val['net_total'] - $val['return_amt']) - $val['receipt_bill'][0]['receipt_paid']), 2);
...
//        }
    ...
    function export_all_quotation_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Invoice ID', 'Firm Name', 'Firm GSTIN', 'Customer name', 'Customer GSTIN', 'Total QTY', 'CGST', 'SGST', 'Sub Total', 'Inv Amt', 'Return Amt', 'Balance Amt', 'INV Date'));
        ...
        foreach ($query[quotation] as $key => $val) {
            ...
            $balance = number_format((($val['net_total'] - $val['return_amt']) - $val['receipt_bill'][0]['receipt_paid']), 2);

            $row = array($key + 1, $val['inv_id'], $val['firm_name'], ($val['gstin']) ? $val['gstin'] : '', ($val['store_name']) ? $val['store_name'] : $val['name'], ($val['tin']) ? $val['tin'] : '', $val['total_qty'], number_format(($val['erp_invoice_details'][0]['cgst']), 2), number_format(($val['erp_invoice_details'][0]['sgst']), 2), number_format($val['subtotal_qty'], 2), number_format($val['net_total'], 2), $return_amount, $balance, ($val['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($val['created_date'])) : '');
...
//            }
    ...
    function export_all_expenses_csv($query, $timezones = array()) {
        ...
        fputcsv($output, array('S.No', 'Customer', 'Invoice ID', 'Category', 'Product', 'Brand', 'Total Qty', 'Inv Amt', 'Return Qty', 'Return Amt', 'CGST', 'SGST', 'Created Date'));
        ...
        foreach ($query as $key => $value) {
            ...
            $row = array($key + 1, $value['store_name'], $value['inv_number'], $value['categoryName'], $value['product_name'], $value['brands'], $value['total_qty'], $value['net_total'], $value['return_qty'], $value['return_amount'], $value['tax'], $value['gst'], ($value['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($value['created_date'])) : '');
