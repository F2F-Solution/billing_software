...
class Sales extends MX_Controller
{
    ...
    public function new_direct_invoice()
    {
        ...
        if ($this->input->post()) {
            ...
            /*
              ...
              if ($advance_amt > $net_total) {

              // Update Customer Advance Amt..

              $this->customer_model->update_customer_details($input['quotation']['customer'], $net_total);

              // Update invoice payment status

              $invoice_id = $insert_id1;

              $this->project_cost_model->update_invoice_payment_status($invoice_id);
                ...
                if (isset($input['categoty']) && !empty($input['categoty'])) {
                    ...
                    foreach ($input['categoty'] as $key => $val) {
                        ...
                        if ($input['quotation']['delivery_status'] == 'delivered') {
                            ...
                            $insert['customer_exists_qty'] = $input['quantity'][$key];
                        }
    ...
    public function update_invoice()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->project_cost_model->delete_return_inv_by_id($input['quotation']['inv_id']);
