        ...
        if ($user_details->num_rows() > 0) {
            ...
            foreach ($user_details as $key => $user_data) {
                ...
                if ($device_log->num_rows() > 0) {
                    ...
                    $result[$user_data['username']][$key] = $user_atten_data;
                }
            }
            if (!empty($result)) {
                $result = ["status" => Success, "message" => "Attendance details found", "result" => $result];
            } else {
                $result = ["status" => fail, "message" => "No data found"];
            }
            return $result;
        } else {
            return false;
